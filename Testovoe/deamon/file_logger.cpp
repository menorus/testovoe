...
bool is_file_log(LogType type) {
    return type == LogType::OPEN || type == LogType::CLOSE ||
        type == LogType::LSEEK || type == LogType::READ ||
        type == LogType::WRITE;
}

int main(int argc, char* argv[]) {
    if (argc < 3) {
        std::cerr << "Usage: file_logger <poll_interval_ms> <log_file>\n";
        return 1;
    }
